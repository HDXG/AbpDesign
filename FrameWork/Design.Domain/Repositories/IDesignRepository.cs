...
namespace Design.Domain.Repositories
{
    public interface IDesignRepository<TEntity,TKey>:IRepository<TEntity, TKey> where TEntity : class, IEntity<TKey>
    {
        ...
        Task<(int, List<TEntity>)> GetPagedListAsync<Key>(Expression<Func<TEntity, bool>> wherePredicate, Func<TEntity, Key> orderPredicate, int pageIndex = 1, int pageSize = 10, bool isReverse = true);
